...
export const TarifValidator: React.FC<TarifValidatorProps> = ({ category }) => {
  ...
  const downloadFullReport = (rowsToDownload?: FullValidationRow[]) => {
    ...
    const csvHeaderArray: string[] = category === 'TARIF' 
    ...
    const csvRowsArray: string[] = dataToExport.map((row: FullValidationRow): string => {
        if (category === 'TARIF') {
            return [
    ...
    const finalCsvString: string = [csvHeaderArray.join(','), ...csvRowsArray].join('\n');
  ...
  const processValidation = async () => {
    ...
    try {
        ...
        const finalValResult: ValidationResult = { 
